lib.gmap = {
	...
	init: function(el, data, callback) {
		...
		/*
			this.init.map.addListener('dragend', function() {
	...
	marker: function(marker, data, hiddenData, callback){
		...
		if(marker){	
			...
			google.maps.event.addListener(marker, 'mouseover', function() {$('#id-'+data.Id+">td").css("background","red")});
	    }
	    callback(marker);
	}
}
